...
public class AuthFrame extends Panel
{
	...
	private Panel loginPanel()
	{
		...
		Label label = new Label("Need account?", 1)
		{
			...
			public void paint(Graphics g)
			{
				...
				g.drawLine(7, this.getHeight() - 6,
						this.getFontMetrics(this.getFont()).stringWidth(this.getText()) + 6, this.getHeight() - 6);
			}
		};
		...
		Panel button = new Panel(new BorderLayout());
		panel.add(button, BorderLayout.SOUTH);
		button.add(this.loginButton, BorderLayout.EAST);
	...
	private Panel offlinePanel()
	{
		...
		Panel text = new Panel(new GridLayout());
		panel.add(text, BorderLayout.CENTER);

		Panel button = new Panel(new BorderLayout());
		panel.add(button, BorderLayout.SOUTH);
		button.add(this.retryButton, BorderLayout.EAST);
		button.add(this.offlineButton, BorderLayout.WEST);
